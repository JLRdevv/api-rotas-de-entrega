...
describe('AuthService', () => {
    ...
    describe('signup', () => {
        it('should create a new user and return JWT token', async () => {
            ...
            expect(usersService.signup).toHaveBeenCalledWith('test@example.com', 'password123');
            expect(jwtService.sign).toHaveBeenCalledWith({ _id: mockUserId.toString() });
        ...
        it('should throw RpcException when user signup fails', async () => {
            ...
            await expect(service.signup('test@example.com', 'password123')).rejects.toThrow(signupError);
        ...
        it('should throw error when JWT signing fails', async () => {
            ...
            await expect(service.signup('test@example.com', 'password123')).rejects.toThrow(jwtError);
    ...
    describe('login', () => {
        it('should authenticate user and return JWT token', async () => {
            ...
            expect(usersService.login).toHaveBeenCalledWith('test@example.com', 'password123');
            expect(jwtService.sign).toHaveBeenCalledWith({ _id: mockUserId.toString() });
            expect(result).toBe(mockToken);
        });
        ...
        it('should throw RpcException when login fails', async () => {
            ...
            await expect(service.login('test@example.com', 'wrongpassword')).rejects.toThrow(loginError);

            expect(usersService.login).toHaveBeenCalledWith('test@example.com', 'wrongpassword');
            expect(jwtService.sign).not.toHaveBeenCalled();
        });
        ...
        it('should throw error when JWT signing fails', async () => {
            ...
            await expect(service.login('test@example.com', 'password123')).rejects.toThrow(jwtError);
